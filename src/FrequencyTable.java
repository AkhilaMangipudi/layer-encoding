...
public final class FrequencyTable {
         ...
         */
        public CodeTree buildCodeTree(int thresholdFrequency) {
                ...
                //Only consider the nodes which have a frequency above the threshold
                for (String i : frequencies.keySet()) {
                        if (frequencies.get(i) > thresholdFrequency) {
                ...
                if(pqueue.size() == 1) {
                        //We need to pad with dummy symbol to add that one symbol to the tree
                        pqueue.add(new NodeWithFrequency(new Leaf("dummy"), "dummy", 0));
                }
